...
class QuestionAnswererNode:
    """질문 답변 노드 (Domain Service 호출하는 얇은 래퍼)
    ...
    async def __call__(self, state: CookingState) -> CookingState:
        ...
        """
        logger.info(f"[Node:QuestionAnswerer] 시작")

        # ✅ Domain Service 호출
        result = await self.cooking_assistant.answer_question(state)

        logger.info(f"[Node:QuestionAnswerer] 완료")

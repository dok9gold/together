...
class RecommenderNode:
    """음식 추천 노드 (Domain Service 호출하는 얇은 래퍼)
    ...
    async def __call__(self, state: CookingState) -> CookingState:
        ...
        """
        logger.info(f"[Node:Recommender] 시작")

        # ✅ Domain Service 호출
        result = await self.cooking_assistant.recommend_dishes(state)

        logger.info(f"[Node:Recommender] 완료: {result.get('dish_names', [])}")

...
# ============ Secondary Intent Result DTO ============

class SecondaryIntentResult(BaseModel):
    """Secondary intent 처리 결과"""
    intent: str = Field(..., description="처리된 intent 이름")
    recipe: Optional[Dict[str, Any]] = Field(None, description="레시피 (recipe_create인 경우)")
    recipes: Optional[List[Dict[str, Any]]] = Field(None, description="복수 레시피 (recipe_create인 경우)")
    recommendations: Optional[List[Recommendation]] = Field(None, description="추천 목록 (recommend인 경우)")
    answer: Optional[str] = Field(None, description="답변 (question인 경우)")
    additional_tips: Optional[List[str]] = Field(None, description="추가 팁 (question인 경우)")
    image_url: Optional[str] = Field(None, description="이미지 URL (generate_image인 경우)")
...
class RecipeResponseData(BaseModel):
    ...
    secondary_results: Optional[List[SecondaryIntentResult]] = Field(None, description="처리된 secondary intent 결과들")
